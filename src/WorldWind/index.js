...
export class Globe extends React.Component {
  componentDidMount() {
    let rect = document.getElementById("worldwind-canvas").getBoundingClientRect()
    ...
    var handleClick = function (recognizer) {
        ...
        let x1 = sprintf("%.2f", p1.latitude), y1 = sprintf("%.2f", p1.longitude)
        let x2 = sprintf("%.2f", p2.latitude), y2 = sprintf("%.2f", p2.longitude)

        let width = parseInt(rect.width - 10, 10), height = parseInt(rect.bottom - 45, 10)
